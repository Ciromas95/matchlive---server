...
app.post("/api/admin/login", (req, res) => {
  if (!ADMIN_PIN) return res.status(500).json({ error: "ADMIN_PIN not configured" });
...
app.get("/api/live", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "API-Football error", details: e?.response?.data ?? e?.message ?? e });
  }
});

app.get("/api/live/compact", async (req, res) => {
  try {
...
// Players flags
// ===============================
app.get("/api/players/flags", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "players_flags_failed" });
  }
});

// ===============================
// League fixtures router
// ===============================
